...
class DQN:
    ''' main DQN class for training model for vanilla and Doubled DQN architecture '''
    
    def __init__(self, action_size = 3, double = False, load_model = False):
        ...
        if self.load_model:
            self.load_model("breakout_dqn.h5")
    ...
    def optimizer(self):
        ''' 
        hacky optimizer with huber loss: 
    ...
    def train(self, total_episodes = 50000):
        ...
        for e in range(total_episodes):
            ...
            if e % 1000 == 0:
                self.save_model("breakout_dqn.h5")

        # closes tf session and env
        self.sess.close()
        env.close()
        
    def save_model(self, name = "breakout_dqn.h5"):
        ''' saves model locally'''
        self.model.save_weights(name)
        
    def load_model(self, name = "breakout_dqn.h5"):
...
if __name__ == '__main__':
    model = DQN(action_size=3, double = True)
